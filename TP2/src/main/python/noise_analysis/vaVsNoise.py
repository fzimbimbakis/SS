...
# Directorios de distintas simulaciones
...
if __name__ == '__main__':
    ...
    for x in range(4):
        for j in [i / 10 for i in range(0, 81, 2)]:
            time, va = read_vas_file("../../resources/noiseAnalysis/" + list[x] + "/vaVsTime(noise="+ str(j)+ ").txt")
            if x == 0:
                if label0:
            ...
            if x == 3:
                if label3:
                    ...
                    label3= True
    plt.legend()
    plt.savefig("../../resources/noise_analysis/graphs/noiseResults.png")